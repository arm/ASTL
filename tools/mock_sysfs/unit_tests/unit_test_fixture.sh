...
if ps -p ${SYSFS_PROCESS} >/dev/null; then
...
if grep -F 'ERROR SUMMARY' "${VALGRIND_LOG}" >/dev/null; then
...
if grep -F 'All heap blocks were freed -- no leaks are possible' "${VALGRIND_LOG}" >/dev/null; then
