...
After a target is configured, ASTL also creates a native event metric for that target named
`astl_pause_events.<target-name>`. This metric is generated by ASTL itself (not by an external
hardware collector) and records the pause markers ASTL emits for that target during collection.

If you discovered metrics before the configure step and cached that list, you must rediscover the
metrics for that target after `astlConfigureMetricCollectionOnTarget(...)` completes if you also
