...
struct MockTopologyManager : public astl::ITopologyManager {
  ...
  using InitializeMetricManagerRtype =
      std::expected<std::unordered_map<astl::ITarget*, std::unique_ptr<astl::IMetricManager>>, astl_status_code>;
  MAKE_MOCK0(ScanForTargets, astl_status_code(), override);
  MAKE_CONST_MOCK1(InitializeMetricManagers, InitializeMetricManagerRtype(const astl::AstlConfiguration& configuration),
                   override);
