...
TEST_CASE("astlGetMetricStates - Finite Set Metric", "[wrapper][MetricStateDiscovery][FiniteSet]") {
  // Setup target
  ...
  labels[astl::AstlValue{static_cast<uint32_t>(0)}] = "Idle";
  labels[astl::AstlValue{static_cast<uint32_t>(1)}] = "Active";
  labels[astl::AstlValue{static_cast<uint32_t>(2)}] = "Sleep";
  ...
  auto finite_set_config = std::make_unique<astl::FiniteSetMetricConfig>(
      ...
      std::move(labels), astl::IdentityFormula{});
  ...
  SECTION("Bad parameters") {
    ...
    REQUIRE(astlGetMetricStates(nullptr, nullptr, nullptr, nullptr) == ASTL_STATUS_BAD_ARGUMENT);
    REQUIRE(astlGetMetricStates(mock_target_handle, nullptr, nullptr, nullptr) == ASTL_STATUS_BAD_ARGUMENT);
    REQUIRE(astlGetMetricStates(mock_target_handle, metric_handle, nullptr, nullptr) == ASTL_STATUS_BAD_ARGUMENT);
    REQUIRE(astlGetMetricStates(mock_target_handle, metric_handle, states.data(), nullptr) == ASTL_STATUS_BAD_ARGUMENT);

    state_count = 0;
    REQUIRE(astlGetMetricStates(mock_target_handle, metric_handle, states.data(), &state_count) ==
  ...
  SECTION("Valid request returns state names and values") {
    ...
    REQUIRE(astlGetMetricStates(mock_target_handle, metric_handle, states.data(), &state_count) == ASTL_STATUS_SUCCESS);
...
TEST_CASE("astlGetMetricStates - Residency Metric", "[wrapper][MetricStateDiscovery][Residency]") {
  // Setup target
  std::vector<std::unique_ptr<astl::ITarget>> mock_targets;
  auto                                        mock_target        = std::make_unique<MockTarget>();
  astl_target_handle_t                        mock_target_handle = mock_target.get();
  ...
  state_info["C0"] = astl::ResidencyMetricConfig::StateInfo{"C0", 1000000, astl::NullOperationBuilder{}};
  state_info["C1"] = astl::ResidencyMetricConfig::StateInfo{"C1", 1000000, astl::NullOperationBuilder{}};
  state_info["C6"] = astl::ResidencyMetricConfig::StateInfo{"C6", 1000000, astl::NullOperationBuilder{}};
  ...
  state_configs.push_back({"C0", 1000000, astl::NullOperationBuilder{}});
  state_configs.push_back({"C1", 1000000, astl::NullOperationBuilder{}});
  state_configs.push_back({"C6", 1000000, astl::NullOperationBuilder{}});
...
TEST_CASE("astlGetMetricStates - Unsupported Metric Type", "[wrapper][MetricStateDiscovery][InvalidType]") {
  // Setup target
  std::vector<std::unique_ptr<astl::ITarget>> mock_targets;
  auto                                        mock_target        = std::make_unique<MockTarget>();
  astl_target_handle_t                        mock_target_handle = mock_target.get();
