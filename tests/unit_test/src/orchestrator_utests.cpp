...
TEST_CASE("Orchestrator-Collection", "[Orchestrator]") {
  ...
  auto orchestrator   = astl::Orchestrator(std::move(collector_manager), std::move(metric_manager));
...
TEST_CASE("Orchestrator-StopCollection", "[Orchestrator]") {
  ...
  SECTION("No metric manager") {
    astl::Orchestrator orchestrator(std::move(collector_manager), nullptr);
    orchestrator.SetTargets(std::move(mock_targets));
  ...
  SECTION("No collector manager") {
    astl::Orchestrator orchestrator(nullptr, std::move(metric_manager));  // cppcheck-suppress accessMoved
    orchestrator.SetTargets(std::move(mock_targets));                     // cppcheck-suppress accessMoved
    auto& target_ptr = orchestrator.GetTargets()[0];
    REQUIRE(orchestrator.StopCollection(target_ptr.get()) == ASTL_STATUS_INTERNAL_ERROR);
  }
  SECTION("valid collector and metrics") {
    astl::Orchestrator orchestrator(std::move(collector_manager),  // cppcheck-suppress accessMoved
...
TEST_CASE("Orchestrator-SinkSamples", "[Orchestrator]") {
  auto orchestrator = astl::Orchestrator(nullptr, nullptr);
  ...
  REQUIRE(orchestrator.SinkSamples(nullptr, samples) == ASTL_STATUS_BAD_ARGUMENT);
}