 ...
 *   2. PAUSE (0) and RESUME (1) can be similarly injected, confirming the internal event values.
 ...
 *   5. The internal LifecycleEventType values remain stable.
 */
...
static void RegisterLifecycleMetric(astl::MetricManager& mgr, const astl::ITarget* target) {
  ...
  auto              cfg         = std::make_unique<astl::EventMetricConfig>(
      metric_name, "ASTL lifecycle events (pause, resume, crop boundary)", ASTL_UNITS_NONE, ASTL_VALUE_UINT64,
      ASTL_METRIC_IDENTIFIER_UNKNOWN, astl::CollectorType::ASTL_NATIVE, astl::NullOperationBuilder{},
      astl::EventMetricConfig::ValueToInfoMap{}, true);
...
TEST_CASE("InjectLifecycleEvent: no-op when no lifecycle metric registered for target", "[MetricManager][lifecycle]") {
  ...
  REQUIRE(mgr.InjectLifecycleEvent(&target, static_cast<uint64_t>(astl::LifecycleEventType::CROP_BEGIN),
...
TEST_CASE("InjectLifecycleEvent: returns BAD_ARGUMENT on null target", "[MetricManager][lifecycle]") {
  astl::MetricManager mgr{MakeCaps()};
  REQUIRE(mgr.InjectLifecycleEvent(nullptr, static_cast<uint64_t>(astl::LifecycleEventType::CROP_BEGIN),
...
TEST_CASE("InjectLifecycleEvent: CROP_BEGIN and CROP_END events carry exact timestamps", "[MetricManager][lifecycle]") {
  ...
  REQUIRE(mgr.InjectLifecycleEvent(&target, static_cast<uint64_t>(astl::LifecycleEventType::CROP_BEGIN), ts_begin) ==
          ASTL_STATUS_SUCCESS);
  REQUIRE(mgr.InjectLifecycleEvent(&target, static_cast<uint64_t>(astl::LifecycleEventType::CROP_END), ts_end) ==
          ASTL_STATUS_SUCCESS);

  REQUIRE(sink.samples.size() == 2);
  CHECK(sink.samples[0].get<uint64_t>() == static_cast<uint64_t>(astl::LifecycleEventType::CROP_BEGIN));
  CHECK(sink.samples[1].get<uint64_t>() == static_cast<uint64_t>(astl::LifecycleEventType::CROP_END));
...
TEST_CASE("InjectLifecycleEvent: PAUSE and RESUME event values match enum constants", "[MetricManager][lifecycle]") {
  ...
  REQUIRE(mgr.InjectLifecycleEvent(&target, static_cast<uint64_t>(astl::LifecycleEventType::PAUSE), timestamp) ==
          ASTL_STATUS_SUCCESS);
  REQUIRE(mgr.InjectLifecycleEvent(&target, static_cast<uint64_t>(astl::LifecycleEventType::RESUME), timestamp) ==
          ASTL_STATUS_SUCCESS);
...
TEST_CASE("InjectLifecycleEvent: independent per-target isolation — events not cross-contaminated",
  ...
  const auto timestamp = astl::ClockMonotonicRaw::now();
  REQUIRE(mgr.InjectLifecycleEvent(&target_a, static_cast<uint64_t>(astl::LifecycleEventType::CROP_BEGIN), timestamp) ==
          ASTL_STATUS_SUCCESS);
  ...
  REQUIRE(mgr.InjectLifecycleEvent(&target_b, static_cast<uint64_t>(astl::LifecycleEventType::CROP_END), timestamp) ==
          ASTL_STATUS_SUCCESS);
  REQUIRE(sink.samples.size() == 2);
  CHECK(sink.samples[1].get<uint64_t>() == static_cast<uint64_t>(astl::LifecycleEventType::CROP_END));
}

TEST_CASE("LifecycleEventType values remain stable", "[lifecycle]") {
  static_assert(static_cast<uint64_t>(astl::LifecycleEventType::PAUSE) == 0, "PAUSE must be 0");
  static_assert(static_cast<uint64_t>(astl::LifecycleEventType::RESUME) == 1, "RESUME must be 1");
  static_assert(static_cast<uint64_t>(astl::LifecycleEventType::CROP_BEGIN) == 2, "CROP_BEGIN must be 2");
  static_assert(static_cast<uint64_t>(astl::LifecycleEventType::CROP_END) == 3, "CROP_END must be 3");
