...
// Reusable test target base implementing ITarget with minimal behavior.
class TestTargetBase : public astl::ITarget {  // NOLINT(cppcoreguidelines-special-member-functions)
 public:
  ...
  auto GetProperties(astl_target_properties_t* properties) const
      -> astl_status_code override {  // NOLINT(readability-non-const-parameter)
    if (!properties) {
  ...
  size_t GetCounterCount() const override {
    return 0;
  }  // NOLINT(readability-convert-member-function-to-static) intentional dynamic dispatch for interface compliance
  auto GetCounters() const -> std::vector<std::unique_ptr<astl::ICounter>> const& override { return counters_; }

 private:
  std::string                                  name_;                                       // initialized via ctor
  astl::CollectorType                          collector_type_{astl::CollectorType::SCMI};  // initialized via ctor
  std::vector<std::unique_ptr<astl::ICounter>> counters_;                                   // empty
};

// Reusable test metric base implementing IMetric with configurable name & units.
class TestMetricBase : public astl::IMetric {  // NOLINT(cppcoreguidelines-special-member-functions)
 public:
  ...
  void SetProcessedSampleSink(astl::IProcessedSampleSink* sink) override {
    (void)sink;
  }  // NOLINT(readability-convert-member-function-to-static)
  void             Reset() override {}
  ...
  astl_status_code GetProperties(
      astl_metric_properties_t* props) const override {  // NOLINT(readability-non-const-parameter)
    if (!props) {
      return ASTL_STATUS_BAD_ARGUMENT;
    }
    props->_handle = this;
 ...
 private:
  std::string  name_;                    // initialized via ctor
  astl_units_t units_{ASTL_UNITS_NONE};  // default units
};

// Legacy ad-hoc helpers replaced by TestTargetBase/TestMetricBase.
}  // namespace
