...
done <"${TMP_DIR}/index_test_dirs.txt"

# Pass 2 - Merge the coverage data from multiple tests suites into one output
find "${TMP_DIR}/finished_merges/" -maxdepth 1 -mindepth 1 >"${TMP_DIR}/index_all_test_suite_dirs.txt"
NUM_TESTSUITE_DIRS=$(wc -l <"${TMP_DIR}/index_all_test_suite_dirs.txt")
...
for CUR_MANGLED_INPUT_FILE in "${MERGED_SUITES_DIR}"/accumulator_in/*; do
