...
struct ITopologyManager {
  ...
  /** @brief Initialize the MetricManager instances for each target based on the configuration and system config files
  ...
  virtual auto InitializeMetricManagers(const AstlConfiguration& configuration) const
      -> std::expected<std::unordered_map<ITarget*, std::unique_ptr<IMetricManager>>, astl_status_code> = 0;
